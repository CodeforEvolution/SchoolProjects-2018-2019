...
public class Deck {
	...
	public Deck(String[] ranks, String[] suits, int[] values) {
		...
		if (ranks.length > values.length)
			throw new IllegalArgumentException("Try to make equal size arrays...");

		for (int rankCount = 0; rankCount < ranks.length; rankCount++)
		{
			for (int suitsCount = 0; suitsCount < suits.length; suitsCount++)
			{
				cards.add(new Card(ranks[rankCount], suits[suitsCount], values[rankCount]));
			}
		}
